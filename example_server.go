...
func runServer() {
	...
	if autoGen, ok := packbuilder.BuildResourcePack(world.CustomItems(), pv419.Ver()); ok && !conf.DisableResourceBuilding {
	...
	conf.Listeners = []func(conf server.Config) (server.Listener, error){
		func(conf server.Config) (server.Listener, error) {
			cfg := minecraft.ListenConfig{
				...
				AcceptedProtocols:      []minecraft.Protocol{pv582, pv419},
			}
	...
	for srv.Accept(func(p *player.Player) {
		...
		p.Inventory().SetItem(1, item.NewStack(block.Air{}, 1))
	}) {
	}
}
