...
type AvailableCommands struct {
	...
	// above.
	Constraints []types.CommandEnumConstraint
}
...
func (pk *AvailableCommands) Marshal(w protocol.IO) {
	...
	for _, command := range pk.Commands {
		types.WriteCommandData(w, &command, enumIndices, suffixIndices, dynamicEnumIndices)
	}
	...
	for _, constraint := range pk.Constraints {
		types.WriteEnumConstraint(w, &constraint, enumIndices, valueIndices)
	}
}
...
func (pk *AvailableCommands) enums() (enums []types.CommandEnum, indices map[string]int) {
	indices = make(map[string]int)

	for _, command := range pk.Commands {
		if len(command.Aliases) > 0 {
			aliasEnum := types.CommandEnum{Type: command.Name + "Aliases", Options: command.Aliases}
...
func (pk *AvailableCommands) dynamicEnums() (enums []types.CommandEnum, indices map[string]int) {
	indices = make(map[string]int)
